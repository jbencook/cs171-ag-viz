...
var detailVis2 = d3.select("#detailVis").append("svg").attr({
    width:350,
    height:200,
    })
    .attr("background_color", "gray")
...
queue()
    ...
    .defer(d3.csv, "../data/county_yield_2000_2013.csv")
    .await(ready)


function ready(error, us, stations, yields) {
    ...
    areaYield(us.objects.counties, yields);
};
...
function areaYield(countyPolygons, yields) {
    county_IDs = {};
    console.log(yields)
    ...
    yields.forEach(function(d) {
        id = "" + parseInt(d["State ANSI"]) + d["County ANSI"]
        if (!(id in county_IDs)) {
            console.log(id);
        } else {
            d3.selectAll(".counties [id='" + id + "']")
                .style("fill", "magenta")
        }
    })
